...
class DefaultParser:
    ...
    def propose(stream=None):
        return {'chunk': 48, 'burst': 1, 'checksum': False}
...
class FlightManagerPointParser1V6(DefaultParser):
    ...
    def propose(stream=None):
        settings = DefaultParser.propose(stream)
        settings.update({'chunk': 51, 'checksum': True})
        return settings
...
class SensorMonitorLogParser(DefaultParser):
    ...
    def propose(stream):
        ...
        if stream.component.swVersion[1] == 5:
            settings = DefaultParser.propose(stream)
            settings.update({'chunk': 240, 'burst': 5})
            return settings
        else:
...
class UavMonitorSessionParser(DefaultParser):
    ...
    def propose(stream):
        settings = DefaultParser.propose(stream)
        settings.update({'chunk': 240, 'checksum': True})
        return settings
...
class PassportStorageParser(DefaultParser):
    ...
    def propose(stream):
        settings = DefaultParser.propose(stream)
        settings.update({'chunk': 64, 'checksum': True})
        return settings
    ...
    def native(self, size):
        if self.cache is None or self.size != size:
            ...
            for i in range(0, int(len(data) / 64)):
                fields = PassportStorageParser.parse(data[i * 64:(i + 1) * 64])
