...
class SystemInfoPage(GenericPage):
    ...
    def drawLicenseInfo(self, top, bottom):
        self.parent.screen.addstr(top[1], top[0], 'License', curses.color_pair(Window.Color.DEFAULT) | curses.A_BOLD)
        width, height = bottom[0] - top[0], bottom[1] - top[1]

        lic = self.parent.hub.getLicense()

        if lic is not None:
            self.parent.screen.addstr(top[1] + SystemInfoPage.MARGIN_TOP + 0, top[0], ' UUID      {:s}'
                .format(str(uuid.UUID(bytes = bytes(lic['uuid'])))))
            self.parent.screen.addstr(top[1] + SystemInfoPage.MARGIN_TOP + 1, top[0], ' Time      {:s}'
                .format(datetime.fromtimestamp(lic['timestamp']).strftime("%F %T")))
            self.parent.screen.addstr(top[1] + SystemInfoPage.MARGIN_TOP + 2, top[0], ' End Time  {:s}'
                .format(datetime.fromtimestamp(lic['endTime']).strftime("%F %T")))
            self.parent.screen.addstr(top[1] + SystemInfoPage.MARGIN_TOP + 3, top[0], ' License Number   {:d}'.format(lic['license']))
            self.parent.screen.addstr(top[1] + SystemInfoPage.MARGIN_TOP + 4, top[0], ' Launches         {:d}'.format(lic['launches']))
            self.parent.screen.addstr(top[1] + SystemInfoPage.MARGIN_TOP + 5, top[0], ' Launches Limit   {}'.format(lic['launchesLimit']))
            self.parent.screen.addstr(top[1] + SystemInfoPage.MARGIN_TOP + 6, top[0], ' Active           {}'.format(bool(lic['active'])))
    ...
    def draw(self):
        ...
        self.drawLicenseInfo((int(3 * width / 4), int(height / 4)), (width, height - 1))
...
args.add_argument('--udp', dest='udp', help='use udp address', default=False, action='store_true')
...
if options.serial != '' and options.address != '':
    ...
    try:
        ...
        if options.udp == True:
            stream = proto.DatagramStream(address, port, modemAddress, modemPort)
        else:
